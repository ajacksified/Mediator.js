...
(function (global, factory) {
  ...
  Channel.prototype.getSubscriber = function (identifier) {
    const parent = this;
    for (let x = 0; x < parent._subscribers.length ; x++) {
      if (parent._subscribers[x].id === identifier || parent._subscribers[x].fn === identifier) {
        return parent._subscribers[x];
      }
    }
  };
  ...
  Mediator.prototype.publish = function (channelName) {
    ...
    argArray.push(channel);
    channel.publish(argArray);
  };
